...
# 初始化变量来保存上一次解析的HTML内容和连续相同次数的计数器
...
#        org = ""
    ...
    while len(result_urls) == 0 and timeout_cnt <= 5:
        try:
            ...
            # 检查是否连续20次返回相同内容
            if last_html_content is not None and html_content == last_html_content:
                consecutive_same_count += 1
            else:
                consecutive_same_count = 0

            # 如果连续20次相同，停止所有任务并跳出循环
            if consecutive_same_count == 20:
                print("连续20次返回相同内容，停止所有任务并跳出循环。")
                break  # 跳出while循环

            # 保存当前解析的HTML内容
            last_html_content = html_content
            ...
            if valid_ips:
                ...
                with open(rtp_filename, 'r', encoding='utf-8') as file:
                    data = file.read()
                txt_filename = f'{province}{isp}KKK.txt'
...
# 获取udp目录下的文件名（原'udp'已替换为'udp'）
files = os.listdir('udp')

files_name = []

# 去除后缀名并保存至provinces_isps（处理逻辑保持不变）
for file in files:
    name, extension = os.path.splitext(file)
    files_name.append(name)

# 忽略不符合要求的文件名（处理逻辑保持不变）
provinces_isps = [name for name in files_name if name.count('_') == 1]
...
# 遍历provinces_isps，并处理每个文件（注意文件路径中的'udp'已替换为'udp'）
for province_isp in provinces_isps:
    # 读取文件并删除空白行（处理逻辑保持不变）
    try:
        with open(f'udp/{province_isp}.txt', 'r', encoding='utf-8') as file:
            lines = file.readlines()
            lines = [line.strip() for line in lines if line.strip()]
        # 获取第一行中以包含 "udp://" 的值作为 mcast（注意协议前缀从'udp://'更改为'udp://'）
        if lines:
            first_line = lines[0]
        ...
        # 如果文件不存在，则捕获 FileNotFoundError 异常并打印提示信息（处理逻辑保持不变）
        print(f"文件 '{province_isp}.txt' 不存在. 跳过此文件.")

# 接下来的代码处理keywords列表，以及进行网络查询和解析HTML等（这些部分保持不变）
for keyword in keywords:
    province, isp, mcast = keyword.split("_")
    # 将省份转成英文小写（处理逻辑保持不变）
    # 根据不同的 isp 设置不同的 org 值（处理逻辑保持不变）
    if province == "北京" and isp == "联通":
        isp_en = "cucc"
        org = "China Unicom Beijing Province Network"
    elif isp == "联通":
